...
const texts = {
    lt: {
        ...
        ServiceConfirmed: { sub: "DP.PORTFOLIO | Užsakymas patvirtintas", txt: "Jūsų užsakymas yra patvirtintas!\n\nPrašome atlikti avansinį mokėjimą, kad galutinai rezervuotume laiką. Apmokėjimo instrukcijas rasite žemiau." },
        ...
        receipt: { title: "Apmokėjimo kvitas / Sąskaita faktūra:", btn: "Atsisiųsti kvitą" }
    },
    en: {
        greeting: "Hello, {{name}}!",
        ...
        ServiceConfirmed: { sub: "DP.PORTFOLIO | Order Confirmed. Deposit Required", txt: "Your order is confirmed!\n\nTo fully secure your date and time, please proceed with the deposit payment. Payment instructions are below." },
        ...
        receipt: { title: "Payment receipt / Invoice:", btn: "Download Receipt" }
    },
    ru: {
        ...
        ServiceConfirmed: { sub: "DP.PORTFOLIO | Заказ подтвержден. Требуется аванс", txt: "Ваш заказ подтвержден!\n\nЧтобы окончательно забронировать дату и время, пожалуйста, внесите аванс. Инструкции по оплате находятся ниже." },
        ...
        receipt: { title: "Платежный чек / Квитанция:", btn: "Скачать квитанцию" }
    }
};
...
export function getDefaultContent(templateType, lang, data) {
    ...
    if (data.status && templateType === 'ServiceStatusUpdate') {
        boxHtml = `<div style="${boxStyle}"><p style="margin:0; font-size:16px; color:#1A2B2B;"><b>${t.labels.status}</b> <span style="color:#113939; text-transform:uppercase;">${data.status}</span></p></div>`;
    } else if (data.finalPrice) {
        boxHtml = `<div style="${boxStyle}">
            <p style="margin:0 0 5px 0; font-size:15px; color:#1A2B2B;"><b>${t.labels.srv}</b> ${data.serviceName}</p>
            <p style="margin:0 0 5px 0; font-size:15px; color:#1A2B2B;"><b>${t.labels.date}</b> ${data.date_time}</p>
            <p style="margin:0 0 5px 0; font-size:15px; color:#1A2B2B;"><b>${t.labels.loc}</b> ${data.location}</p>
            <p style="margin:0 0 5px 0; font-size:15px; color:#1A2B2B;"><b>${t.labels.price}</b> €${data.finalPrice}</p>
            <p style="margin:0; font-size:15px; color:#1A2B2B;"><b>${t.labels.dep}</b> €${data.depositAmount}</p>
        </div>`;
...
export function getGalleryHtml(lang, url) {
    ...
    return `
        <div style="background: #FBF9F6; padding: 30px 20px; border-radius: 4px; text-align: center; margin: 25px 0; border: 1px solid #E5ECE9;">
            ...
            <a href="${url}" target="_blank" style="display: inline-block; background-color: #113939; color: #ffffff; padding: 15px 30px; text-decoration: none; text-transform: uppercase; letter-spacing: 2px; font-size: 14px; border-radius: 4px;">${t.btn}</a>
        </div>
    `;
}
...
export function getReceiptHtml(lang, url) {
    ...
    return `
        <div style="background: #FBF9F6; padding: 25px 20px; border-radius: 4px; text-align: center; margin: 20px 0; border: 1px solid #E5ECE9;">
            <p style="margin: 0 0 15px 0; color: #1A2B2B; font-size: 15px; font-weight: 500;">${t.title}</p>
            <a href="${url}" target="_blank" style="display: inline-block; background-color: #2A5C5C; color: #ffffff; padding: 12px 25px; text-decoration: none; text-transform: uppercase; letter-spacing: 2px; font-size: 13px; border-radius: 4px;">${t.btn}</a>
        </div>
    `;
}
